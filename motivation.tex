...
To better explain our categorization, we now walk through some representative examples for each category.
...
We found an example of refactoring for complexity in a Cilium commit that involved changing how the code handled IPv4 fragmentation.
...
We now see how both of these fixes were needed in implementing BMC~\cite{BMC}, an in-kernel cache for Memcached
    based on eBPF.
...
BMC suffers from verifier complexity in its cache
