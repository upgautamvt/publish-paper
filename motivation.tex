...
Another common fix for verifier complaints involves adding code to please the verifier.
...
Each of these categories involves writing specific code to pass the verifier.
...
\emph{Explicitly teaching the verifier information}
...
\emph{Implementing functions in a specific way}
The aya-rs project had to implement versions of \verb{memset{ and \verb{memcpy{ which are very simple for the verifier.
As can be seen in Figure~\ref{fig:aya-memset} the code is as basic as you could get, but it is necessary for the verifier.
