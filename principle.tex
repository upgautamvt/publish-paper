...
\subsection{Bound program runtimes}
...
Previous works~\cite{ebpf-termination,untenableVerification} have also
    ...
    verification limits, further wakening the runtime guarrantee from the
    verifier.
...
One challenge involved in terminating a \projname{} program safely is that a
    ...
    \projname{}.
To address this challenge, \projname{} delays termination when the program is
    running kernel helper functions, and does not attempt to terminate if the
    program is in the panic handler, which already means graceful exit.
% \begin{enumerate}
...
\begin{figure}
    \includegraphics[width=1.0\linewidth]{figs/BPF_termination_state_diagram.png}
    \centering
    %\vspace{-10pt}
...
The states are: 1) executing programs' own code, 2) executing kernel helpers or
...
When excuting the timeout termination handler, if the flag is at state 2),
    ...
    terminated, the termination handler just modifies it to state 3) without
