...
\begin{itemize}
	\item A \projname{} program should never be terminated while executing a helper function
...
we use a per-cpu tristate flag which can only have states denoting : (i) within \projname{},
...
the termination handler just modifies it to state (iii). 
On a helper function return, the flag is checked and if set, the panic handler is called. 
