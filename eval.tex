...
The complexity is exacerbated by introducing two additional variables to track
    the state of the key, the SET command.
...
In the \projname{}-BMC, specific checks such as the ones for identified SET commands and the \texttt{key\_found}
    state are being translated to utilize built-in functions and closures.
...
The check on \texttt{BMC\_MAX\_PACKET\_LENGTH}, served as a constraint to minimize the number
    of jump instructions thereby circumventing eBPF verifier, is no longer necessary
    in the Rust code.
...
\jinghao{Just realized that we are not using any special syntactic sugar here}
% \jinghao{The four levels of nesting in the original code is significantly...}
...
Due this limit, BMC has to be split into seven seperated eBPF program, while
    ideally only two are required (egress and ingress hooks).
...
In the \projname{}-BMC, due to the absence of the program size limit,
...
%     which significantly obscure the logic of program and may undermine the performance.
    programs no longer have to put the metadata in a seperated map, which leads
...
We evaluate on how the in-kernel memory footprint of \projname{} programs
...
The number of pages allocated for and the actual amount used by the eBPF
...
%     say ``project''}
Before Linux 5.18, each eBPF program occupies its own pages.
