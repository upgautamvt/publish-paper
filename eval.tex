...
The equivalent cache invalidation code in \projname{} is shown in
    Figure~\ref{fig:rust-code}.
    In the \projname{}-BMC, specific checks such as those for identified SET commands and the \texttt{key\_found}
    state are being translated to utilize built-in lambda functions and closures.
\jinghao{We will need to show where the \texttt{key\_found} and \texttt{set\_found} are assigned in the
original C code. We should also talk about \texttt{BMC\_MAX\_PACKET\_LENGTH}.}
Meanwhile, other checks, including those for offset and \texttt{data\_end} limits, are being restrict via
...
For \projname{}, this includes all \texttt{LOAD} segments from the compiled ELF
    executable.
A compiled \projname{} program generally has four \texttt{LOAD} segments, which
...
We also include the BMC program (\S~\ref{eval:bmc-case-study}) in the
    experiment because it represents an example of a more complicated eBPF
    program.
We implement a \projname{} version for each of the programs.

The number of pages allocated for and the fractions occupied by of the eBPF
    programs and their \projname{} counterparts are shown in
...
%     say ``project''}
Before Linux 5.18, each eBPF program is loaded independently and
    occupies its own pages.
eBPF introduced the ``packed allocation'' feature since Linux 5.18, which
    tries to pack different programs into the same page to use memory more
    efficiently and is labeled in Figure~\ref{fig:eval-mem-footprint} as
    ``BPF-Packed''.
For \projname{}, multiple programs in the same project are
    loaded together in the same executable.
However, different segments of the executable have to reside in different
...
When comparing to eBPF without packed allocation, the situation depends on the
    number of programs in the same project.
...
\texttt{tracex5} and \texttt{syscall\_tp} uses the same number of memory pages
    since each of them consists of 4 programs, and one page per program is
    allocated for without packed allocation.
