...
\jinghao{Need data for discussion, don't know how large the overhead would be.}
...
We selected three widely-used eBPF map types for this experiment: array maps,
    hash maps, and per-CPU array maps.
Among them, array maps and hash maps supports inlining while per-CPU array maps
    does not.
...
For maps that support lookup inlining, i.e. array maps and hash maps, the
...
The implementation for the BMC is kind of wired.
\jinghao{@Ruowen should talk about some experience in implementing BMC, especially
